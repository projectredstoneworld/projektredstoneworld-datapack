...
execute unless entity @s[nbt={Motion:[0.0,0.0,0.0]}] if score @s rcrspeedupspeed matches 0 rotated as @s as @e[type=pig,x=0,tag=rcrailseatf] if score @s redstoneworldID = @e[type=minecart,tag=rcraildrive,tag=handleSeatMovement,limit=1,sort=nearest] redstoneworldID run tp @s ^-1.5 ^5 ^
execute unless entity @s[nbt={Motion:[0.0,0.0,0.0]}] if score @s rcrspeedupspeed matches 0 rotated as @s as @e[type=pig,x=0,tag=rcrailseatb] if score @s redstoneworldID = @e[type=minecart,tag=rcraildrive,tag=handleSeatMovement,limit=1,sort=nearest] redstoneworldID run tp @s ^-3.5 ^5 ^
