...
execute at @s unless entity @e[tag=rcsmanager,distance=..30,type=marker] if score @s rcrspeedupspeed matches -1 rotated as @s as @e[type=pig,x=0,tag=rcrailseatf] if score @s redstoneworldID = @e[type=minecart,tag=rcraildrive,tag=handleSeatMovement,limit=1,sort=nearest] redstoneworldID run tp @s ^0.5 ^5 ^
execute at @s unless entity @s[tag=rcsmanager,distance=..30,type=marker] if score @s rcrspeedupspeed matches -1 rotated as @s as @e[type=pig,x=0,tag=rcrailseatb] if score @s redstoneworldID = @e[type=minecart,tag=rcraildrive,tag=handleSeatMovement,limit=1,sort=nearest] redstoneworldID run tp @s ^-2.5 ^5 ^
