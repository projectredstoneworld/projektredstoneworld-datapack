...
tag @e[tag=rcraildrive,type=minecart,distance=..1,limit=1,sort=nearest,tag=!rcrailghost] add rcrailaged
...
execute if score @s rcrailunpowertime matches 10..20 run tag @e[type=minecart,tag=rcraildrive,tag=rcrailghost,distance=..32] add rcrailaged
