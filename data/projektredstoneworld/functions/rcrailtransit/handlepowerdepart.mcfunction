...
execute if score @s rcrailunpowertime matches 0 run tag @e[tag=rcraildrive,type=minecart,distance=..1,limit=1,sort=nearest] add closingrcr
execute if score @s rcrailunpowertime matches 0 at @e[type=minecart,tag=rcraildrive,tag=closingrcr,distance=..10] run function projektredstoneworld:rcrailtransit/doorclose1
