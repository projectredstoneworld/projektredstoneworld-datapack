...
execute store result score #limboslot info run data get entity @s data.Inventory[0].Slot
...
execute as @a[tag=restoretargetlimbo] run function projektredstoneworld:rtcreac/give_correct_slotlimbo

# remove item data from entity
data remove entity @s data.Inventory[0]
# count down the remaining slots
scoreboard players remove #restorelimboitems info 1
# run the same function again if there are more items to process
execute if score #restorelimboitems info matches 1.. run function projektredstoneworld:rtcreac/return_item_limbo
