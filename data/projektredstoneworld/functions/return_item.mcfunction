...
execute store result score #slot info run data get entity @s data.Inventory[0].Slot
...
execute as @a[tag=restoretarget] run function projektredstoneworld:give_correct_slot

# remove item data from entity
data remove entity @s data.Inventory[0]
# count down the remaining slots
scoreboard players remove #restoreitems info 1
# run the same function again if there are more items to process
execute if score #restoreitems info matches 1.. run function projektredstoneworld:return_item
