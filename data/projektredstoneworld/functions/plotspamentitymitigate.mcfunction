...
execute in plotworld as @e[type=armor_stand,x=0] at @s if score @s plotentityspamcheck matches 25.. run tellraw @a[distance=..20] {"text":"Warning: Excessive entity spam nearby detected and reported, further violations may result in action.","color":"red"}
execute in plotworld as @e[type=#projektredstoneworld:vehicles,x=0] at @s if score @s plotentityspamcheck matches 25.. run tellraw @a[distance=..20] {"text":"Warning: Excessive entity spam nearby detected and reported, further violations may result in action.","color":"red"}
